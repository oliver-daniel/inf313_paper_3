...
subsetWith('year', 'sexnow', 'sexornt', sel = T) |>
  ...
  labs(
    title = "Demographics of transgender and non-binary GSS respondents across 2018 and 2021 surveys" |> str_wrap(60),
    x = "Gender identity of respondent",
    y = "Count",
    fill = "Sexual orientation of respondent"
  )
...
subsetWith('sexbirth', 'sexnow') |>
  mutate(
    code = paste0(birth_codes[sexbirth], gender_codes[sexnow]),
    ) |>
  select(code, sex) |>
  table() |>
  knitr::kable(col.names = c('Recorded as male', 'Recorded as female')) |>
  kableExtra::kable_classic(full_width = FALSE)
```

```{r fig-sexual-partners}
library(patchwork)
...
makePlot('ptnrornt') + labs(title = '"My latest sexual partner was..."') + theme(legend.position = "bottom", legend.direction = "vertical", axis.title.y = element_blank(), axis.title.x = element_blank(), legend.title = element_blank(), axis.ticks.y = element_blank())
```
```{r}
# ((makePlot('sexsex') + theme(axis.title.y = element_blank())) +
# (makePlot('sexsex5') + theme(axis.text.y = element_blank(), axis.title.y = element_blank()))) +
